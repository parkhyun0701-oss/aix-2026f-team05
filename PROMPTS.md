...
**참고**
지시, 규약(CONVENTIONS.md), 근거(스키마, 기존 서비스/라우터 코드), 종료조건을 통합한 프롬프트를 제공하여 별도 사람이 수정할 필요 없이 요구사항을 100% 준수하는 코드가 생성됨.

-----------------------------------------------------------------------------------------------------------------------------

## 2026-09-17 · AI 사용 기록 / AI use log (3주차 활동)

### 아이디어 A

#### 1건
- 무엇을 하려고 썼는가
> 아이디어 A의 최종 수정안(UI 기반 커스텀 파이프라인 및 RAG)에 맞는 검증 가정을 세우고, 인터뷰 질문 3개 추출

- 넣은 프롬프트 원문 그대로:
> 아이디어 A에 대해 이를 검증하기 위해 추천하는 방법 2~3가지를 선정해 작성해 줘 인터뷰, 관찰, 기존데이터, 직접 해보기 중에서. 인터뷰를 진행한다면 과거 경험을 묻는 질문 3개 정도?, 직접 해보기를 한다면 측정할 숫자와 상황을 포함해 주고

- 나온 것 중 쓴 것 / 버린 것:
> 쓴 것: 인터뷰 및 직접 해보기(타임어택) 방법 선정, 과거형으로 작성된 3가지 인터뷰 질문 및 측정 지표(소요 시간, 클릭 수)
> 버린 것: 사용자 관찰(Observe) 방법론 및 학사 통계 데이터 활용안

- 버렸다면 왜 버렸는가
> 대학생의 개인 노트북 내 정리 습관이나 툴 사용 포기 경험은 관찰이나 기존 통계 데이터로는 파악하기 어렵고, 실제 인터뷰와 직접 해보기가 훨씬 명확한 수치를 제공하기 때문

#### 2건
- 무엇을 하려고 썼는가
> 3절의 '지금은 어떻게 하고 있는가(What do they do today?)' 항목 생성을 위한 기존 대안 및 포기 행동 패턴 도출

- 넣은 프롬프트 원문 그대로:
>아이디어 A의 사용자들이 현재 도구가 없을 때 문제는 어떻게 해결하고 있는지(기존 대안 및 포기 행동), 그리고 그것이 왜 충분하지 않은지 정리해 줘

- 나온 것 중 쓴 것 / 버린 것:
> 쓴 것: "필요할 때마다 LMS에 들어가서 하나씩 열어본다(포기함)"라는 행동 양식과 과목 증가에 따른 불충분성 논리
> 버린 것: "Make/Zapier 같은 자동화 툴을 유료로 구독해서 쓴다"는 대안
...
1건
- 무엇을 하려고 썼는가
> 아이디어 c의 후보군에서 그밖의 사용자와 사용할 수 없는 사용자
- 넣은 프롬프트 원문 그대로:
> 사용자를 말고 다른 사람은 누가 있지? 그리고 쓰고 싶어도 못 쓰는 사람은?

- 나온 것 중 쓴 것 / 버린 것:
> 쓴 것: 다른 사용자들, 사용하지 않는 사람들의 종류
> 버린 것: 그 사용자들이 사용하거나 사용하지 않는 이유
- 버렸다면 왜 버렸는가
...
#### 1건
- 무엇을 하려고 썼는가
> 아이디어의 문제가 실제로 있는지 증명하기 위한 인터뷰 질문제작

- 넣은 프롬프트 원문 그대로:
> 이 아이디어를 증명하기 위해 인터뷰를 해야 한다고 가정하면, 어떤 질문을 해야 할까?

- 나온 것 중 쓴 것 / 버린 것:
> 쓴 것: 과거 경험을 묻는 질문 3개(최근 북마크를 다시 찾은 방법, 찾지 못해 포기한 경험, 다른 정리 도구를 쓰다 그만둔 경험)
> 버린 것: "북마크 동기화 기능이 켜져 있는지 아는가" 질문, "이런 서비스가 있다면 쓰겠는가" 질문

- 버렸다면 왜 버렸는가
> 동기화 기능을 아는지는 지식을 묻는 질문이라 "저장해도 안 보인다"는 문제를 확인하지 못하고, 서비스를 쓰겠냐는 질문은 미래 의향이라 대부분 긍정적으로 답해 증거가 되지 않기 때문
